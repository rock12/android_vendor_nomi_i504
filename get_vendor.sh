...
/lib/libui_ext.so /lib/libui.so /lib/libdrmmtkutil.so /lib/libdrmmtkwhitelist.so /lib/libdrmmtkutil.so"
...
/lib/libbluetoothdrv.so /lib/hw/audio.usb.default.so /lib/libtinycompress.so /lib/libtinyxml.so /lib/libMtkOmxCore.so \
/lib/libMtkOmxVenc.so /lib/libMtkOmxVdec.so /etc/mtk_omx_core.cfg /lib/libaudiomtkdcremoval.so /lib/hw/audio.primary.mt6580.so \
...
/lib/libmnl.so /lib/libstagefrighthw.so /lib/libmtcloader.so /bin/permission_check"
